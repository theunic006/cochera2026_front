...
const CompanyList = () => {
  ...
  // ✅ OPTIMIZACIÓN: Estado para búsqueda con debounce
  ...
  // ✅ OPTIMIZACIÓN: useCallback - Cargar estados disponibles
  ...
  // ✅ OPTIMIZACIÓN: useCallback - Función para cargar empresas
  const loadCompanies = useCallback(async (page = 1, pageSize = 15) => {
    setLoading(true);
    try {
      let response;
  ...
  // ✅ OPTIMIZACIÓN: useCallback - Manejar filtro por estado
  ...
  }, [loadCompanies, pagination.pageSize]);

  // ✅ OPTIMIZACIÓN: useCallback - Limpiar filtros
  const clearFilters = useCallback(() => {
    setStatusFilter('');
    loadCompanies(1, pagination.pageSize);
  }, [loadCompanies, pagination.pageSize]);

  // ✅ OPTIMIZACIÓN: useCallback - Crear nueva empresa
  const handleCreateCompany = useCallback(() => {
    setEditingCompany(null);
    setCompanyFormVisible(true);
  }, []);

  // ✅ OPTIMIZACIÓN: useCallback - Editar empresa
  const handleEditCompany = useCallback((company) => {
    setEditingCompany(company);
    setCompanyFormVisible(true);
  }, []);

  // ✅ OPTIMIZACIÓN: useCallback - Eliminar empresa
  const handleDeleteCompany = useCallback(async (id, nombre) => {
    try {
  ...
  // ✅ OPTIMIZACIÓN: useCallback - Cambiar estado de empresa
  const handleChangeStatus = useCallback(async (id, nombre, newStatus) => {
    try {
      let response;
      if (newStatus === 'activo') {
  ...
  // ✅ OPTIMIZACIÓN: useCallback - Callback cuando se crea/edita exitosamente
  ...
  ], [handleEditCompany, handleDeleteCompany, handleChangeStatus]);
