...
const IngresoMobile = () => {
  ...
  // ✅ OPTIMIZACIÓN: useCallback - Actualizar ingreso
  const handleUpdateIngreso = useCallback(async (values) => {
  ...
  }, [ingresoSeleccionado, form, cargarIngresos, searchText]);

  // ✅ OPTIMIZACIÓN: useCallback - Terminar ingreso con pago
  const handleTerminarIngreso = useCallback(async (tipoPago) => {
    try {
      setLoading(true);
  ...
  }, [ingresoSeleccionado, user, cargarIngresos, searchText]);

  // ✅ OPTIMIZACIÓN: useCallback - Obtener clases CSS basadas en el tema
  const getThemeClass = useCallback((baseClass) => {
    return `${baseClass} ${isDarkMode ? 'dark' : 'light'}`;
  }, [isDarkMode]);

  // ✅ OPTIMIZACIÓN: useCallback - Renderizar item de la lista
  const renderIngresoItem = useCallback((ingreso, index) => {
  ...
  }, [toleranciaMinutos, handleEditIngreso, handleTerminar, getThemeClass]);
