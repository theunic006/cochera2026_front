...
import { companyService } from "../../empresas/services/companyService";
...
const ConfigImpresora = ({ 
  ...
  const handleLoadPrintersOnEdit = useCallback(async () => {
    const now = Date.now();
  ...
  }, [printersLoaded, cacheTime, loadPrinters]);

  // Función para guardar configuración de impresoras
  const handleSavePrinters = useCallback(async (values) => {
    try {
      // Verificar token
