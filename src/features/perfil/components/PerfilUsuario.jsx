...
const PerfilUsuario = () => {
  ...
  const handleSuccessPasswordChange = useCallback((updatedUserData) => {
    setShowPasswordModal(false);
    // No necesita actualizar datos del usuario, solo la contraseña
  }, []);

  const handleSuccessEditProfile = useCallback((updatedUserData) => {
  ...
  if (loading) {
    return (
      <AppLayout>
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh', color: isDark ? '#fff' : '#000' }}>
          Cargando perfil...
        </div>
      </AppLayout>
    );
