...
const OwnerList = () => {
  ...
  /**
   * ✅ OPTIMIZACIÓN: useCallback - Cargar propietarios desde la API
   */
  const loadOwners = useCallback(async (page = 1, pageSize = 15) => {
    setLoading(true);
    try {
  ...
  }, [loadVehiclesCount]); // ✅ Depende solo de loadVehiclesCount

  /**
   * ✅ OPTIMIZACIÓN: useCallback - Buscar propietarios
   */
  const handleSearch = useCallback(async (value, page = 1, pageSize = 15) => {
    setSearchText(value);
  ...
  }, [loadOwners, loadVehiclesCount]); // ✅ Depende de loadOwners y loadVehiclesCount

  /**
   * ✅ OPTIMIZACIÓN: useCallback - Manejar cambio de página y tamaño de página
   */
  const handleTableChange = useCallback((newPagination) => {
    // No actualizar el estado aquí - dejamos que loadOwners lo haga
    if (isSearching && debouncedSearchText) { // ✅ OPTIMIZACIÓN: Usar debouncedSearchText
      // Si estamos buscando, usar el endpoint de búsqueda
      handleSearch(debouncedSearchText, newPagination.current, newPagination.pageSize);
    } else {
      // Cargar propietarios normalmente
      loadOwners(newPagination.current, newPagination.pageSize);
    }
  }, [isSearching, debouncedSearchText, handleSearch, loadOwners]); // ✅ Dependencias correctas

  /**
   * ✅ OPTIMIZACIÓN: useCallback - Abrir modal para crear propietario
   */
  ...
  /**
   * ✅ OPTIMIZACIÓN: useCallback - Abrir modal para editar propietario
   */
  ...
  /**
   * ✅ OPTIMIZACIÓN: useCallback - Eliminar propietario
   */
  const handleDeleteOwner = useCallback(async (ownerId, ownerName) => {
    try {
  ...
  }, [isSearching, debouncedSearchText, handleSearch, loadOwners, pagination.current, pagination.pageSize]);

  /**
   * ✅ OPTIMIZACIÓN: useCallback - Manejar éxito en formulario (crear/editar)
   */
  ...
  /**
   * ✅ OPTIMIZACIÓN: useCallback - Cargar cantidad de vehículos
   */
  const loadVehiclesCount = useCallback(async (ownersList) => {
    const counts = {};
  ...
  /**
   * ✅ OPTIMIZACIÓN: useCallback - Abrir modal de vehículos
   */
  const handleViewVehicles = useCallback((owner) => {
  ...
  ], [pagination.current, pagination.pageSize, vehiclesCount, handleViewVehicles, handleEditOwner, handleDeleteOwner]); // ✅ OPTIMIZACIÓN: Dependencias del useMemo
